...
import com.k_office.data.response.AuthTypeResponse
...
interface KOfficeApiService {
    ...
    suspend fun sendOtp(@Body request: SendOtpRequest): Response<AuthTypeResponse>

    @POST(ConstUrls.VERIFY_OTP)
    suspend fun verifyOtp(@Body request: VerifyOtpRequest): OtpResponse

    @POST(ConstUrls.VERIFY_REGISTER)
    suspend fun verifyRegister(@Body request: VerifyOtpRequest): Response<MessageResponse>
